      ...
      <main>
        <div className="md:grid grid-cols-3 md:w-4/5 mx-auto">
          <section className="flex flex-col mx-auto md:order-last col-span-2 md:col-span-1">
            <PhotoImage
              name={current.name}
              image={current.links.image}
              email={current.links.email}
              scholar={current.links.scholar}
              youtube={current.links.youtube}
          ...
          <AboutInformation
            ...
            motivationTitle={current.motivationTitle}
            motivationA={current.motivationA}
            motivationB={current.motivationB}
          />
        </div>
      </main>
      <div>
...
export async function getStaticProps(context) {
  ...
  let current = await fs.readFile(`data/people/current/${data}.json`, "utf-8");
  return {
    props: { current: JSON.parse(current) },
  };
}
